...
from app.parsers import parse_model, parse_ue_method, Estimator

app = Flask(__name__)

model: Optional[Model] = None
ue_methods: Dict[str, Estimator] = {}
cache_path: str = '/Users/romanvashurin/cache'

DEFAULT_DATA = {
    'type': 'single',
    'model': 'BART-aeslc',
    'ue': ['sequence-level, md'],
}
...
def generate():
    ...
    for method in methods:
        uncertainties.append([x for x in processor.ue_estimations[level, str(method)]])
    ...
    for t in processor.stats['greedy_tokens'][0][:-1]:
        tokens.append(model.tokenizer.decode([t], skip_special_tokens=True))
    if len(tokens) > 0:
