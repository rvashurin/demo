...
app.post('/get-prompt-result', async (req, res) => {
    ...
    try {
        ...
        if (!token_level) {
            ...
            response += Math.round(uncertainty[0]);
            response += '</span>';
        }

        if (generation.length != 0) {
            response += '\n<span style="color: rgb(178, 190, 181); font-size: 12px">Uncertainty estimation: MD-Encoder</span>';
            response += '\n<span style="color: rgb(178, 190, 181); font-size: 12px">Model: BART-aeslc</span>';
        }
